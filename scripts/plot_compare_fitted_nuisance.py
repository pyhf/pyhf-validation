...
def plot_compare_fitted_nuisance(files, output):
    ...
    for filename in files:
        for p_param, p_pyhf, p_root, p_abs, p_rel in np.loadtxt(
            files[0],
            dtype={
                "names": ["param", "pyhf", "root", "abs", "rel"],
                "formats": ["|S42", float, float, float, float],
            },
            skiprows=2,
        ):
    ...
    data_abs, data_rel = list(
        zip(*[i for k, v in data.items() for i in zip(v["abs"], v["rel"])])
    )
    fig, ax = plt.subplots(2, 1, figsize=(10, 20))
    ax[0].hist(np.abs(data_rel), bins=np.logspace(0, 5, 50))
    ax[0].set_xscale("log")
    ax[0].set_xlabel(
        r"$\left|\ \frac{\mathrm{pyhf} - \mathrm{ROOT}}{\mathrm{pyhf}}\ \right|$"
    )
