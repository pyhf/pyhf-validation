...
def process_patch(p):
    ...
    stat_deltas = np.zeros_like(histo_deltas)      # Don't consider the stat error for this calculation
...
def plot_rel_systs(p, channel_names, channel_bins):
    ...
    sys_names_histo = [np.asarray([m['name'] for m in p['value']['modifiers'] if m['type'] == 'histosys'])][0].tolist()
    hid = np.asarray([m['data']['hi_data'] for m in p['value']['modifiers'] if m['type'] == 'histosys'])
    lod = np.asarray([m['data']['lo_data'] for m in p['value']['modifiers'] if m['type'] == 'histosys'])
    delta_up = hid-nom
    delta_dn = nom-lod
    ...
    sys_names_norm = [np.asarray([m['name'] for m in p['value']['modifiers'] if m['type'] == 'normsys'])][0].tolist()
    hi = np.asarray([m['data']['hi'] for m in p['value']['modifiers'] if m['type'] == 'normsys'])
    lo = np.asarray([m['data']['lo'] for m in p['value']['modifiers'] if m['type'] == 'normsys'])
    delta_up = np.asarray([h*nom-nom for h in hi])
    delta_dn = np.asarray([l*nom-nom for l in lo])
    ...
    for iBin in range(n_bins):
        ...
        sys_names_bin = [sys_names[idx] for idx, size in enumerate(rel_size) if size > 0.5 and np.isfinite(size)]     # Select for relative systematics above 0.5 (i.e. >50% variation from nominal)
        ...
        if len(sys_names_bin) > 0:
            ...
            plt.tight_layout()
            plt.savefig(f'Plots/rel_systs_{signal_name}_{channel_name}_bin{bin_number}.png')
            plt.close()
...
def outlier_plot(signal_template, v_max, x_var, y_var, x_label, y_label):
    ...
    for ichan, channel in enumerate(channels_json):
        ...
        for jbin in range(2,2+channel_bins[channel]):
            ...
            bin_number = jbin - 1
            rel_systs = rel_systs[rel_systs[:, jbin] != 0]   # Remove any points with zero relative syst
            
            if len(rel_systs) > 0:
              z = scipy.interpolate.griddata(rel_systs[:,:2],rel_systs[:,jbin],(x,y))
              #print(rel_systs)
              
              vmin,vmax = 0, v_max
              #z = np.log(z)
              ax.scatter(rel_systs[:,0],rel_systs[:,1], c = rel_systs[:,jbin], edgecolors='w', vmin = vmin, vmax = vmax)
